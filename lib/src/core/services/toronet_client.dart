...
class ToronetClient {
  ...
  void switchNetwork(Network newNetwork) {
    ...
    _sdk = ToronetSDK(
      ...
      customConnectWUrl: newNetwork == Network.mainnet
          ? 'https://restapi.connectw.com/api'
          : ApiUrl.testbaseUrl,
      dio: _createDio(),
    );
            ...
            if (location != null) {
              if (location.startsWith('/')) {
                ...
                final portStr =
                    ...
                    : '';
                location =
                    '${originalUri.scheme}://${originalUri.host}$portStr$location';
              }
              ...
              try {
                ...
                if (newResponse.data is String) {
                  try {
                    ...
                    newResponse.data = {
                      'result': false,
                      'error': newResponse.data,
                    };
                  }
                }
                if (newResponse.data is List) {
                  newResponse.data = newResponse.data.isNotEmpty
                      ? newResponse.data.first
                      : {};
                }
