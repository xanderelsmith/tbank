...
void main() async {
  ...
  void findCurrencyRequests(dynamic item, String path) {
    if (item is Map) {
      final name = item['name'] ?? '';
      final currentPath = path.isEmpty ? name : '$path / $name';
      if (currentPath.contains('/currency')) {
      ...
      if (item.containsKey('item')) {
        findCurrencyRequests(item['item'], currentPath);
      }
    } else if (item is List) {
      for (final subItem in item) {
        findCurrencyRequests(subItem, path);
      }
    }
  }
  
  findCurrencyRequests(data, '');
}
